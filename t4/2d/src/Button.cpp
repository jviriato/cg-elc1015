...
Button::Button(int x, int y, int width, int height, string l, double r, double g, double b)
{
    setColor(r, g, b);
    setPosition(x, y);
...
void Button::render()
{
    double middle_of_the_box = getY() + (getHeight() / 2);
    // drawCollisionBox();
    if (isInside)
    {
        color(1, 0, 0);
    ...
    if (isPressed)
    {
        vector<double> rgb = RGBtodouble(115, 101, 152);
